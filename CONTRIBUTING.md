...
- `make clean integration-test ARGS=--json` will rebuild the binary and then execute it with json output
...
- `npx editorconfig-checker .` - Check for `.editorconfig` confrormance
- All the above checks can be run with `make check`
- Any automatic fixes can be done with `make fix` ([editorconfig-checker does not currently have automatic fixes](https://github.com/editorconfig-checker/editorconfig-checker/issues/14).)
